...
@implementation NSFollowersDatasource
...
-(void) loadData{
    ...
    NSURL *URL=[NSURL URLWithString:[NSString stringWithFormat:@"%@?webservice=ui&action=getfollowers",[NSGlobalConfiguration URL]]];
...
-(void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error{
    ...
    if([Delegate respondsToSelector:ErrorSelector]){
        [Delegate followersdatasource:self FailedWithError:error];
    }
...
-(void) parser:(NSXMLParser *)parser foundCharacters:(NSString *)string{
    if(ErrorDetected){
        ...
        NSError *error=[[NSError alloc] initWithDomain:@"FollowersAPI" code:200 userInfo:ErrorDictionary];
        SEL ErrorSelector=@selector(followersdatasource:FailedWithError:);
        if([Delegate respondsToSelector:ErrorSelector]){
            [Delegate followersdatasource:self FailedWithError:error];
            [parser abortParsing];
...
-(void) parser:(NSXMLParser *)parser parseErrorOccurred:(NSError *)parseError{
    SEL ErrorSelector=@selector(followersdatasource:FailedWithError:);
    if([Delegate respondsToSelector:ErrorSelector]){
        [Delegate followersdatasource:self FailedWithError:parseError];
    }
...
-(void) parser:(NSXMLParser *)parser validationErrorOccurred:(NSError *)validationError{
    SEL ErrorSelector=@selector(followersdatasource:FailedWithError:);
    if([Delegate respondsToSelector:ErrorSelector]){
        [Delegate followersdatasource:self FailedWithError:validationError];
    }
}
-(void) parserDidEndDocument:(NSXMLParser *)parser{
    ...
    if([Delegate respondsToSelector:CompletionSelector]){
        [Delegate followersdatasourceFinishedLoading:self];
    }
}
@end
